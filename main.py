...
st.write(f"總負債：{total_debt:,}")

cash_on_hand = df_balance_sheet.loc["cashAndCashEquivalents", 0]
st.write(f"現金及現金等價物：{cash_on_hand:,}")

restricted_cash = st.number_input('限制性現金', value=0)

market_capital = market_cap[0]["marketCap"]
st.write(f"市值：{market_capital:,}")
...
st.write(f"5年營收成長率 {round(growth_rate_revenue_5*100,2)}%")
st.write(f"8年營收成長率 {round(growth_rate_revenue_8*100,2)}%")
st.write(f"10年營收成長率 {round(growth_rate_revenue_10*100,2)}%")
...
st.write(f"5年每股盈餘成長率 {round(growth_rate_eps_5*100,2)}%")
st.write(f"8年每股盈餘成長率 {round(growth_rate_eps_8*100,2)}%")
st.write(f"10年每股盈餘成長率 {round(growth_rate_eps_10*100,2)}%")

growth_fcf_share_5 = (fcf_share/(df_cash_flow.loc["freeCashFlow",5]/(df_income.loc["netIncome",5]/df_income.loc["epsdiluted",5])))**(1/5)-1
growth_fcf_share_8 = (fcf_share/(df_cash_flow.loc["freeCashFlow",8]/(df_income.loc["netIncome",8]/df_income.loc["epsdiluted",8])))**(1/8)-1
growth_fcf_share_10 = (fcf_share/(df_cash_flow.loc["freeCashFlow",10]/(df_income.loc["netIncome",10]/df_income.loc["epsdiluted",10])))**(1/10)-1
st.write(f"5年每股自由現金流成長率 {round(growth_fcf_share_5*100,2)}%")
st.write(f"8年每股自由現金流成長率 {round(growth_fcf_share_8*100,2)}%")
st.write(f"10年每股自由現金流成長率 {round(growth_fcf_share_10*100,2)}%")
